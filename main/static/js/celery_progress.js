...
class CeleryProgressBar {
    ...
    constructor(progressUrl, options) {
        ...
        let barColorsDefault = {
            success: '#1ed75ff2;', // spotify green
            error: '#dc143c', // crimson
            progress: "#1ed75ff2", // spotify green
            ignored: '#888'
        }
        ...
        let defaultMessages = {
            waiting: 'Calling Spotify API...',
            started: 'Hitting Spotify API....',
        }
    ...
    onSuccessDefault(progressBarElement, progressBarMessageElement, result) {
        ...
        progressBarMessageElement.innerHTML = "Enjoy :)"
    }

    onResultDefault(resultElement, result) {
        if (resultElement) {
            resultElement.innerHTML = `<a target="_blank" href="${result}">${result}</a>`;
        }
    }
